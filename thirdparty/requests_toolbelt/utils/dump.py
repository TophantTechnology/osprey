...
def _get_proxy_information(response):
    if getattr(response.connection, 'proxy_manager', False):
        ...
        # TCC-PATCH: parse request_url and get path instead of full_request_path in HTTP header line
        request_url = compat.urlparse(request_url).path
...
def _build_request_path(url, proxy_info):
    ...
    if proxy_url is not None:
        proxy_url = _coerce_to_bytes(proxy_url)    # TCC-PATCH: force proxy_url to bytes
